...
int gr_joint_cmd(lua_State* L)
{
  ...
  luaL_argcheck(L, lua_rawlen(L, 2) == 3, 2, "Three-tuple expected");
  luaL_checktype(L, 3, LUA_TTABLE);
  luaL_argcheck(L, lua_rawlen(L, 3) == 3, 3, "Three-tuple expected");
...
int gr_cylinder_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  double radiustop = luaL_checknumber(L, 2);
  double radiusbottom = luaL_checknumber(L, 3);

  data->node = new GeometryNode(name, new Cylinder(radiustop, radiusbottom));
...
int gr_material_cmd(lua_State* L)
{
  ...
  luaL_argcheck(L, lua_rawlen(L, 1) == 3, 1, "Three-tuple expected");
  luaL_checktype(L, 2, LUA_TTABLE);
  luaL_argcheck(L, lua_rawlen(L, 2) == 3, 2, "Three-tuple expected");
...
int gr_texture_cmd(lua_State* L)
{
  ...
  data->texture = new Texture(string(name));
  if( !data->texture->Load() )
  {
	cerr << "ERROR: Failed to load texture: " << name << endl;
  }
